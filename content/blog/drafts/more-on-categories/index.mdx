...
arrow is the source of the next (so they can be composed together). By parallel paths we mean all the paths have the same first sources
and the same final targets.
...
Given a set $S$, we can consider its power set $P(S),$ which is the set of all subsets of $S$, as a preorder. If $X$ and $Y$ are subsets of $S$, and thus
elements of $P(S)$, we say $X ≤ Y$ if $X$ is a subset of $Y$, which we write as $X \subseteq Y$.
...
[^1]:
    The way of combining resources is referred to as a _monoidal product_, which we haven’t discussed before, but the details
...
[^2]:
    As an example of the importance of symmetries, Noether’s theorem says that if a physical system has a (continuous) symmetry,
